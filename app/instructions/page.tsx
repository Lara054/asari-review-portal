...
type Item = {
  ...
  tags: string[];
  date: string; // ISO
  href: string; // 例: "/instructions/sample.html"
  slug: string;
  file: string;
};
...
export default function InstructionsPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await fetch("/instructions/index.json", { cache: "no-store" });
        if (!res.ok) {
  ...
  return (
    <div className="space" style={{ display:"grid", gap: "16px" }}>
      ...
      <div style={{display:"flex",gap:16,alignItems:"center",justifyContent:"space-between",flexWrap:"wrap"}}>
        <div>
          <div style={{display:"flex",gap:12,alignItems:"center"}}>
            <Link href="/" style={{fontSize:20,textDecoration:"none",color:"#0066ff"}}>← 戻る</Link>
            <h1 style={{margin:0,fontSize:22,fontWeight:700,letterSpacing:".2px"}}>動画作成指示書一覧</h1>
          </div>
          <p style={{margin:"6px 0 0",color:"var(--muted)",fontSize:13}}>
            <code>public/instructions/</code> にHTMLを置くと自動反映されます。
          </p>
        </div>
      ...
      {!error && filtered.length===0 && (
        <div className="empty">該当する指示書がありません。</div>
      )}
    </div>
  );
}
